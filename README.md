...
- **共享方法论**：正反双方与中立主审都必须读取内置方法论；编译版同样包含这些文件。
...
仓库中的两份方法论会被 PyInstaller 打进编译产物：

- `methodology/辩论方法论.md`
- `methodology/逻辑与论辩学理论.md`

建立一场辩论时，程序把两份文件分别复制到每个阵营和主审的隔离工作目录，并在初始化 prompt 中要求完整读取：

- **双方备赛**使用破题、资料分级、论点构建、证据核验和攻防预演框架。
- **双方场上发言**使用质询、驳论、自由辩论、总结陈词和逻辑谬误检查。
- **中立主审**使用图尔明模型、证据五维度、论证负担和深化方法统一点评。

构建脚本会在打包前检查两份文件；缺失时直接停止构建，避免出现“源码有方法论、编译版没有”的静默退化。
